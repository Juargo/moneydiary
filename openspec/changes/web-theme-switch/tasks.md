...
- [x] 10.0 **Manual QA gate (before continuing to S7b):** run `pnpm web dev`, confirm dark Sin categoría bucket fill `#696C63` reads distinctly against the card and the ingreso-tint cell (tightest measured margins, `palette-measurements.md`). Done by the orchestrator via Playwright screenshots (stubApi): dark dashboard (Sin categoría wedge and the selected-month mini-pie ring) and the four light screens; reviewed and approved by the product owner before PR11.
