...
- [x] 22.3 **Partially performed (2026-09-08) — HTTP layer only, no browser.** Smoke run against a real dev API (port 3100) plus the local disposable Postgres, using `apps/api/test/fixtures/pdf/protegida-test.pdf` (password `clave-fixture-pdf-protegido-2026`). Verified outside the test harness:
  - `POST /api/ingestas/preview` with no password → `400 {"code":"PDF_PROTEGIDO"}`.
  - Wrong password → `400 {"code":"PDF_PASSWORD_INCORRECTA"}` — **distinct codes reach the client**, confirming the Phase 12 preview error-channel fix works outside unit tests.
  - Correct password → the PDF **decrypts successfully** (the password gate is passed).
  - **D-09 carve-out against a REAL database**: 3 consecutive `POST /api/ingestas/commit` calls with a wrong password wrote **0 rows** to `Ingesta`.
  - **Inverse control**: a non-password failure (`no-banco-test.pdf`) **did** write its `FALLIDA` row — proving the carve-out is neither too broad nor too narrow.
- [ ] 22.5 **OPEN GAP 1 — the full happy path has never been exercised.** With the correct password the fixture decrypts but then fails with `BancoNoReconocidoError`, because `protegida-test.pdf` carries only the marker text `"PDF PROTEGIDO FIXTURE"` — it is not a bank statement. The extractor unit tests are correct for their layer (they only need tokens), but **no test and no manual run has ever taken a protected REAL cartola through to a preview with transactions**. Closing this needs an encrypted fixture built from a genuine statement, which is materially harder than hand-rolling a minimal PDF. Track before relying on this path in production.
- [ ] 22.6 **OPEN GAP 2 — the React UI has never run in a browser.** The reactive reveal, the retry that reuses the already-selected `File`, and the user-facing copy are covered only by jsdom tests. A human or a Playwright pass should still drive `pnpm web dev` + `pnpm api dev` end to end.
- [x] 22.4 Re-confirmed the proposal's Success Criteria checklist item by item — all 7 items hold: (1) locked PDF → protected error, never `PdfInvalidoError` (Slice 1/3); (2) correct password completes preview AND commit, typed once (Slice 2/4) — **holds at unit level only; see 22.5, the end-to-end happy path with a real cartola is still unproven**; (3) wrong password → no `Ingesta` row (Slice 2 D-09 carve-out); (4) password never in log/error/response/DB/browser storage (D-07 layers 1-4, this slice's 21.1/21.2); (5) unlocked PDF and `.xlsx` paths unchanged (baselines preserved: api 268/2547, web 141/1791, both only additive); (6) no password field until API reports protected (20.1); (7) `pnpm api test`/`pnpm web test`/`tsc --noEmit` all green (`openapi.json` drift-check unchanged since this slice touched no backend/contract files — last verified clean in Slice 3).
